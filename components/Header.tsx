...
import { ChevronDown, PhoneIcon, PlayCircleIcon } from "lucide-react";
...
const Header = () => {
  const [mobileisopne, setIsmobileopen] = useState(false);

  return (
    <header className=" bg-[url('/Back.jpg')]">
      ...
      aria-label="Global">
        <div className="flex lg:flex-1">
          <Link href="/" className="flex items-center ">
            ...
            height={35}
            />
          <h2 className="font-semibold text-[#FFD700] text-lg pt-2">Booking</h2>
          </Link>
        </div>
                  ...
                  {products.map((item) => (
                    ...
                    >
                      <div className="flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-gray-200">
                        <item.icon
                          className="h-6 w-6 text-[#0c0b0b] group-hover:text-blue-600"
                          aria-hidden="true"
                        />
                      </div>
                ...
                <div className="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
                  {callsToAction.map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      className="flex items-center justify-center gap-x-2.5 p-3 text-sm font-semibold leading-6 text-[#000000] hover:bg-gray-100"
                    >
                      <item.icon
                        className="h-5 w-5 flex-none text-[#030303]"
                        aria-hidden="true"
                      />
                      {item.name}
                    </a>
